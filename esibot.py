...
zombini_EDT_url = "https://edt.zombini.fr/controler/edt.ctrl.php?classe=1A-TP1&page=1"
...
def GetWeekEDT(config):
    ...
    if datetime.today().weekday() <= 4: ##Si c'est le weekend (samedi = 5)
    ...
    week = [lundi, mardi, mercredi, jeudi, vendredi]#[currentday:5]
    ...
    for i in range(5):
        ...
        for matiere in edt[edtday]: #Pour chaque matière
            ...
            if name in config['weekedt_replacename']:
                ...
                name = name.strip()
            color = (175, 175, 175)
            if edt[edtday][matiere]['Title'] in MatieresColorDictionary:
                color = MatieresColorDictionary[edt[edtday][matiere]['Title']]
